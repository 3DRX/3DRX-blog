...
_Consider the network fragment shown below. x has only two attached neighbors, w and y. w
...
in the network have strictly positive integer values._

![](../../../assets/computer_networking/c5p7.png)

_a. Give x’s distance vector for destinations w, y, and u._

_b. Give a link-cost change for either c(x, w) or c(x, y) such that x will inform its neighbors of
a new minimum-cost path to u as a result of executing the distance-vector algorithm._

_c. Give a link-cost change for either c(x, w) or c(x, y) such that x will not inform its neighbors
of a new minimum-cost path to u as a result of executing the distance-vector algorithm._
...
_Consider Figure 5.7. Suppose there is another router w, connected to router y and z. The
costs of all links are given as follows: $c(x,y)=4$, $c(x,z)=50$, $c(y,w)=1$, $c(z,w)=1$,
$c(y,z)=3$. Suppose that poisoned reverse is used in the distance-vector routing algorithm._

![](../../../assets/computer_networking/c5p11.png)

_a. When the distance vector routing is stabilized, router w, y, and z inform their distances to
x to each other. What distance values do they tell each other?_

_b. Now suppose that the link cost between x and y increases to 60. Will there be a count-to-
...
reach a stable state again? Justify your answer._

_c. How do you modify $c(y, z)$ such that there is no count-to-infinity problem at all if $c(y,x)$
changes from 4 to 60?_
...
_Consider the following network. ISP B provides national backbone service to regional ISP
...
point? To answer this question, you will need to dig into the BGP ­specification._
...
_Consider the two ways in which communication occurs between a managing entity and a
...
(3) robustness with respect to lost messages between the managing entity and the device?_
...
_In Section 5.7 we saw that it was preferable to transport SNMP messages in unreliable
UDP datagrams. Why do you think the designers of SNMP chose UDP rather than TCP as the
transport protocol of choice for SNMP?_
